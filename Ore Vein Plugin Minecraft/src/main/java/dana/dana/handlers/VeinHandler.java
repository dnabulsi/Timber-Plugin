...
import dana.dana.OreVeinDestroyer;
...
public class VeinHandler implements Listener {
    public VeinHandler(OreVeinDestroyer plugin) {
            ...
            for (int x = -1; x <= 1; x++) {
                for (int y = -1; y <= 1; y++) {
                    for (int z = -1; z <= 1; z++) {
                        ...
                        if (relativeBlock.getType().toString().contains("ORE") && !blocksFound.contains(relativeBlock)) {
